...
def ShowCalendarHelper.get_calendar_src
 require 'open-uri' 
 require 'iconv'
...
def ShowCalendarHelper.process_calendar(calendar_src)
...
def ShowCalendarHelper.get_month_chunks(calendar_src)
  months = []
...
def ShowCalendarHelper.create_table_rows(activities)
  rows = ""
...
@@switch = 0
def ShowCalendarHelper.cycle(strA, strB)
  if(@@switch == 0)
    @@switch = -1
  end
  if(@@switch == -1)
...
def ShowCalendarHelper.shorten_url(url)
  pattern = /<a.*>(\S*)<\/a>/um
  url =~ pattern
...
def ShowCalendarHelper.shorten_mail_adr(adr)
  pattern = /\S*(@|\sved\s)\S*/
  adr =~ pattern
...
def ShowCalendarHelper.process_activity(activity_chunk, month)
  fields = []
...
def ShowCalendarHelper.process_month(month_chunk)
...
def ShowCalendarHelper.format_error(error)
...
def ShowCalendarHelper.scrape_calendar
  begin
